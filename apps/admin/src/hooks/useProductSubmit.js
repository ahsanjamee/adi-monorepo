...
const useProductSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const updatedVariants = variants.map((v, i) => {
        const newObj = {
          ...v,
          price: Number(getNumberTwo(v?.price)),
          originalPrice: Number(getNumberTwo(v?.originalPrice)),
          discount: Number(getNumberTwo(v?.discount)),
      ...
      if (updatedId) {
        ...
        if (isCombination) {
          ...
          setValue("features", res.features);
          setValue("technicalSpecifications", res.technicalSpecifications);
          setValue("supports", res.supports);
  ...
  useEffect(() => {
    if (!isDrawerOpen) {
      ...
      clearErrors("technicalSpecifications");
      clearErrors("supports");
      clearErrors("stock");
    ...
    setVariants((pre) =>
      pre.map((com, i) => {
        if (i === id) {
          ...
          if (name === "price") {
            updatedCom.price = Math.ceil(value);
