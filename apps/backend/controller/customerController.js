...
const { sendOtp, verifyOtp } = require("../lib/services/OtpServices");
const { createUser } = require("../lib/services/UserServices");
...
const verifyPhoneNumberForLogin = async (req, res) => {
  const { phone } = req.body;
...
const verifyOtpForResetPassword = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ message: "Phone verified, please set new password" });
  } catch (err) {
    res
      .status(500)
      .send({ message: "Error verifying OTP", error: err.message });
  }
};
...
const verifyOtpAndRegister = async (req, res) => {
  ...
  try {
    ...
    res.status(201).send({ message: "Phone verified, please login now", user });
  } catch (err) {
    res
      .status(500)
...
const loginCustomer = async (req, res) => {
  ...
  try {
    ...
    if (!existingUser) {
      ...
      user = await createUser({ name: "User", phone });
    }
    ...
    res.status(500).send({
      message: err.message || "Error during the process",
    });
  }
};
