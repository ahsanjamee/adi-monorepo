...
const updateProductPrices = async (categoryId, discount) => {
  try {
    ...
    for (const product of products) {
      ...
      product.prices.price = discountedPrice;
      product.prices.discount = originalPrice - discountedPrice;

      // Check and update prices for each variant
      if (product.variants && Array.isArray(product.variants)) {
        for (const variant of product.variants) {
          if (variant.originalPrice) {
            const variantOriginalPrice = variant.originalPrice;
            const variantDiscountedPrice =
              variantOriginalPrice - (variantOriginalPrice * discount) / 100;

            // Update the variant price and discount
            variant.price = variantDiscountedPrice;
            variant.discount = variantOriginalPrice - variantDiscountedPrice;
          }
        }
        // Ensure Mongoose tracks changes in the nested array
...
const updateCategory = async (req, res) => {
  try {
    ...
    if (category) {
      ...
      await updateProductPrices(req.params.id, req.body.discount);
